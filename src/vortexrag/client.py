...
from vortexrag.cli import log_server_banner
from vortexrag.mcp_exceptions import (
    NodeNotInstalledError,
    NodeVersionTooLowError,
    check_node_version,
)
from vortexrag.mcp_logging import get_logger
...
def launch_ui(
    ...
    if not frontend_dist.exists() or not (frontend_dist / "index.html").exists():
        ui_logger = logging.getLogger("VortexRAG-UI")
        ui_logger.error(
        ...
        raise RuntimeError(
            "Frontend dist not found. Build required before launching UI."
        )
    ...
    ui_logger.info("VortexRAG UI (%s mode) started: http://%s:%d", mode_str, host, port)

    try:
        app.run(host=host, port=port, debug=False)
    except OSError as exc:
        raise RuntimeError(
            f"Failed to start VortexRAG UI (host={host}, port={port}): {exc}"
        ) from exc
...
def launch_case_study(
    ...
    case_logger = logging.getLogger("VortexRAG-CaseStudy")
    case_logger.info(
...
class UltraData:
    """Data manager for VortexRAG pipeline execution.
        ...
        for i, elem in enumerate(skeleton):
            if elem_match(elem, parent_pairs):
                try:
                    ...
                    raise ValueError(
                        "[VortexRAG Error] Router sub_list length < expected matches when padding to skeleton"
                    )
    ...
    def _extract_io(self, pipeline: List[Any]) -> None:
        ...
        for pipe in pipeline:
            if isinstance(pipe, str):
                ...
                for _, input_val in tool_input.items():
                    if input_val.startswith("$"):
                        ...
                        if stripped not in self.local_vals[srv_name]:
                            raise ValueError(
                                f"[VortexRAG Error] Variable {stripped} not found in {srv_name} parameter.yaml"
                            )
                        ...
                        if mem_name.startswith("memory_"):
                            ...
                            if input_val not in self.global_vars.keys():
                                raise ValueError(
                                    f"[VortexRAG Error] Variable {input_val} cannot be found from pipeline before {srv_name}.{tool_name} step"
                                )
                ...
                for output_val in tool_output:
                    if output_val.startswith("$"):
                        ...
                        if not output_val in self.local_vals[srv_name]:
                            raise ValueError(
                                f"[VortexRAG Error] Variable {output_val} not found in {srv_name} parameter.yaml"
                            )
                ...
                raise ValueError(f"[VortexRAG Error] Unrecognized pipeline step: {pipe}")
                    ...
                    for i, elem in enumerate(full_list):
                        if elem_match(elem, parent_pairs):
                            try:
                                ...
                                raise ValueError(
                                    f"[VortexRAG Error] Prompt {var_name} length < "
            ...
            if len(output_keys) > 0:
                ...
                for key in iter_keys:
                    if not key.replace("$", "") in data:
                        raise ValueError(
                            f"[VortexRAG Error] Output key {key} not found in data for step {server_name}.{tool_name}"
                        )
                    if key.startswith("$"):
                        if key[1:] in self.local_vals[server_name]:
                            ...
                            raise ValueError(
                                f"[VortexRAG Error] Variable {key[1:]} not found in {server_name} parameter.yaml"
                            )
                                ...
                                for i, elem in enumerate(full_list):
                                    if elem_match(elem, parent_pairs):
                                        try:
                                            ...
                                            raise ValueError(
                                                f"[VortexRAG Error] Router {key} length < global_vars in step {server_name}.{tool_name}"
                                            )
                                        full_list[i]["data"] = new_elem
                                if any(True for _ in it):
                                    raise ValueError(
                                    ...
                                    for i, elem in enumerate(full_list):
                                        if elem_match(elem, parent_pairs):
                                            try:
                                                ...
                                                raise ValueError(
                                                    f"[VortexRAG Error] Router {key} length < global_vars"
                                                )
        ...
        if path.endswith(".py"):
            if not os.path.exists(path):
                raise FileNotFoundError(
                    f"[VortexRAG Error] Cannot find the server file of {name}: {path}"
                )
            mcp_servers[name] = {
                "command": "python",
                "args": [path],
            ...
            if not node_status:
                try:
                    ...
                    logger.error(
                        "[VortexRAG Error] Node.js is not installed or not found in PATH. Please install Node.js >= 20."
                    )
                    logger.error(str(e))
                    sys.exit(1)
                except NodeVersionTooLowError as e:
                    logger.error(
                        "[VortexRAG Error] Node.js version is too low. Please upgrade to Node.js >= 20."
                    )
                    logger.error(str(e))
                    sys.exit(1)
            mcp_servers[name] = (
            ...
            raise ValueError(
                f"[VortexRAG Error] Unsupported server type for {name}: {path}"
            )
        ...
        for step in steps:
            if isinstance(step, str):
                ...
                if "loop" in step:
                    ...
                    raise ValueError(
                        f"[VortexRAG Error] Unrecognized step in branch: {step}"
                    )
            else:
                raise ValueError(f"[VortexRAG Error] Unrecognized pipeline step: {step}")
...
def load_pipeline_context(
    ...
    if param_path is not None:
        ...
        if param_config_path is None:
            raise FileNotFoundError(
                f"[VortexRAG Error] Parameter file '{provided_path}' does not exist"
            )
...
async def execute_pipeline(
    ...
    """Execute VortexRAG pipeline with given context.
...
async def run(
    ...
    """Run VortexRAG pipeline with given configuration.
...
def main() -> None:
    """Main entry point for VortexRAG CLI."""
    parser = argparse.ArgumentParser(prog="ultrarag", description="VortexRAG CLI")
    ...
    p_show_ui = show_sub.add_parser("ui", help="Launch the VortexRAG web UI")
