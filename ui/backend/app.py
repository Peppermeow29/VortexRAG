...
FRONTEND_DIR = BASE_DIR.parent / "frontend" / "dist"  # Point to Vue build output
...
def create_app(admin_mode: bool = False) -> Flask:
    ...
    def handle_http_error(err: HTTPException) -> tuple[Dict[str, str], int]:
        """Preserve HTTP status codes (e.g. 404/405) as JSON responses."""
        return (
            jsonify({"error": err.name or "HTTP Error", "details": err.description}),
            err.code or 500,
        )
    ...
    @app.route(
        "/api/kb/files/<string:category>/<string:filename>/rename",
        methods=["POST", "PUT"],
    )
    def rename_kb_file(category: str, filename: str) -> Response:
        """Rename a knowledge base item's display name."""
        if category not in ["raw", "corpus", "chunks", "collection", "index"]:
            return jsonify({"error": "Invalid category"}), 400

        payload = request.get_json(force=True) if request.is_json else {}
        display_name = str(
            payload.get("display_name") or payload.get("new_name") or ""
        ).strip()
        if not display_name:
            return jsonify({"error": "display_name is required"}), 400

        try:
            return jsonify(pm.rename_kb_file(category, filename, display_name))
        except Exception as e:
            return jsonify({"error": str(e)}), 500
    ...
    def run_kb_task() -> Response:
        ...
        kb_config = pm.load_kb_config()
        chunk_defaults = kb_config.get("chunk", {})
        if not isinstance(chunk_defaults, dict):
            chunk_defaults = {}
        embedding_defaults = kb_config.get("embedding", {})
        if not isinstance(embedding_defaults, dict):
            embedding_defaults = {}

        chunk_params = {
            "chunk_backend": payload.get(
                "chunk_backend", chunk_defaults.get("chunk_backend", "token")
            ),
            "tokenizer_or_token_counter": payload.get(
                "tokenizer_or_token_counter",
                chunk_defaults.get("tokenizer_or_token_counter", "gpt2"),
            ),
            "chunk_size": payload.get(
                "chunk_size", chunk_defaults.get("chunk_size", 500)
            ),
            "use_title": payload.get(
                "use_title", chunk_defaults.get("use_title", True)
            ),
        }
        try:
            chunk_params["chunk_size"] = int(chunk_params["chunk_size"])
        except Exception:
            chunk_params["chunk_size"] = 500

        # Embedding parameters (for milvus_index)
        embedding_params = {
            "api_key": payload.get(
                "emb_api_key", embedding_defaults.get("api_key", "")
            ),
            "base_url": payload.get(
                "emb_base_url",
                embedding_defaults.get("base_url", "https://api.openai.com/v1"),
            ),
            "model_name": payload.get(
                "emb_model_name",
                embedding_defaults.get("model_name", "text-embedding-3-small"),
            ),
        }
...
def build_ai_system_prompt(context: Dict) -> str:
    ...
    base_prompt = """You are an AI assistant for VortexRAG, a RAG (Retrieval-Augmented Generation) pipeline configuration system.
