...
KB_COLLECTION_DISPLAY_NAMES_PATH = KB_ROOT / "collection_display_names.json"
...
SESSION_TIMEOUT_SECONDS = int(os.getenv("VORTEXRAG_SESSION_TIMEOUT", "86400"))
BACKGROUND_SESSION_TIMEOUT_SECONDS = int(
    os.getenv("VORTEXRAG_BG_SESSION_TIMEOUT", "172800")
)
...
class DemoSession:
    ...
    def run_chat(
        ...
        # Reset cancellation flag at start of new task
        ...
        try:
            ...
            self._current_future = None
            self._cancel_requested = False  # Reset flag after task completes

    def interrupt_task(self):
        self.touch()
        self._cancel_requested = True  # Set cancellation flag
        if self._current_future and not self._current_future.done():
            self._current_future.cancel()
            LOGGER.info(f"Interrupt signal sent to session {self.session_id}")
            return True
        LOGGER.warning(f"No active task to interrupt for session {self.session_id}")
        return False
...
def chat_demo_stream(
    ...
    def run_bg():
        try:
            ...
            # Catch CancelledError, treat as user-initiated stop
            LOGGER.info(f"Chat background task cancelled for session {session_id}.")
            # Check if cancellation was requested
            if session.is_cancelled():
                LOGGER.info("Cancellation was user-requested, not saving results.")
...
def _kb_default_config() -> Dict[str, Any]:
    return {
        "milvus": {
...
def load_kb_config() -> Dict[str, Any]:
    default_config = _kb_default_config()
    ...
    try:
        ...
        merged = dict(saved)
        for key, section_default in default_config.items():
            current_value = saved.get(key, {})
            if isinstance(section_default, dict):
                section_cfg = dict(section_default)
                if isinstance(current_value, dict):
                    section_cfg.update(current_value)
                merged[key] = section_cfg
            elif key not in merged:
                merged[key] = section_default
...
def save_kb_config(config: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(config, dict):
        raise ValueError("KB config must be a JSON object")

    merged = load_kb_config()
    for key, value in config.items():
        if isinstance(value, dict) and isinstance(merged.get(key), dict):
            section_cfg = dict(merged.get(key, {}))
            section_cfg.update(value)
            merged[key] = section_cfg
        else:
            merged[key] = value

    defaults = _kb_default_config()

    milvus_cfg = merged.get("milvus", {})
    if not isinstance(milvus_cfg, dict):
        milvus_cfg = {}
    full_milvus_cfg = dict(defaults["milvus"])
    full_milvus_cfg.update(milvus_cfg)
    merged["milvus"] = full_milvus_cfg

    chunk_cfg = merged.get("chunk", {})
    if not isinstance(chunk_cfg, dict):
        chunk_cfg = {}
    try:
        chunk_size = int(chunk_cfg.get("chunk_size", defaults["chunk"]["chunk_size"]))
    except Exception:
        chunk_size = defaults["chunk"]["chunk_size"]
    if chunk_size <= 0:
        chunk_size = defaults["chunk"]["chunk_size"]
    merged["chunk"] = {
        "chunk_backend": str(
            chunk_cfg.get("chunk_backend", defaults["chunk"]["chunk_backend"]) or ""
        )
        or defaults["chunk"]["chunk_backend"],
        "tokenizer_or_token_counter": str(
            chunk_cfg.get(
                "tokenizer_or_token_counter",
                defaults["chunk"]["tokenizer_or_token_counter"],
            )
            or ""
        )
        or defaults["chunk"]["tokenizer_or_token_counter"],
        "chunk_size": chunk_size,
        "use_title": bool(chunk_cfg.get("use_title", defaults["chunk"]["use_title"])),
    }

    embedding_cfg = merged.get("embedding", {})
    if not isinstance(embedding_cfg, dict):
        embedding_cfg = {}
    merged["embedding"] = {
        "api_key": str(
            embedding_cfg.get("api_key", defaults["embedding"]["api_key"]) or ""
        ),
        "base_url": str(
            embedding_cfg.get("base_url", defaults["embedding"]["base_url"]) or ""
        )
        or defaults["embedding"]["base_url"],
        "model_name": str(
            embedding_cfg.get("model_name", defaults["embedding"]["model_name"]) or ""
        )
        or defaults["embedding"]["model_name"],
    }
...
def list_kb_files() -> Dict[str, List[Dict[str, Any]]]:
    ...
    collections = []
    collection_display_map = _load_collection_display_names_map()
    db_status = "unknown"
    try:
        ...
        for name in names:
            ...
            if name in collection_display_map:
                display_name = collection_display_map[name]
            collections.append(
                {
                    "name": name,
        ...
        stale_keys = set(collection_display_map.keys()) - names_set
        if stale_keys:
            for stale_key in stale_keys:
                collection_display_map.pop(stale_key, None)
            _save_collection_display_names_map(collection_display_map)
        client.close()
        db_status = "connected"
    except Exception as e:
...
def _load_collection_display_names_map() -> Dict[str, str]:
    """Load display names for Milvus collection names from local mapping file."""
    if not KB_COLLECTION_DISPLAY_NAMES_PATH.exists():
        return {}
    try:
        data = json.loads(KB_COLLECTION_DISPLAY_NAMES_PATH.read_text(encoding="utf-8"))
        ...
        LOGGER.warning(f"Failed to load collection display names map: {e}")
        return {}


def _save_collection_display_names_map(name_map: Dict[str, str]) -> None:
    """Persist collection display names mapping."""
    try:
        KB_COLLECTION_DISPLAY_NAMES_PATH.parent.mkdir(parents=True, exist_ok=True)
        KB_COLLECTION_DISPLAY_NAMES_PATH.write_text(
            json.dumps(name_map, ensure_ascii=False, indent=2), encoding="utf-8"
        )
    except Exception as e:
        LOGGER.warning(f"Failed to save collection display names map: {e}")


def _register_collection_display_name(collection_name: str, display_name: str) -> None:
    """Register display name for a Milvus collection."""
    normalized_collection = str(collection_name or "").strip()
    normalized_display = str(display_name or "").strip()
    if not normalized_collection or not normalized_display:
        return
    name_map = _load_collection_display_names_map()
    name_map[normalized_collection] = normalized_display
    _save_collection_display_names_map(name_map)


def _remove_collection_display_name(collection_name: str) -> None:
    """Remove collection display name mapping if exists."""
    normalized_collection = str(collection_name or "").strip()
    if not normalized_collection:
        return
    name_map = _load_collection_display_names_map()
    if normalized_collection in name_map:
        name_map.pop(normalized_collection, None)
        _save_collection_display_names_map(name_map)
...
def rename_kb_file(category: str, filename: str, display_name: str) -> Dict[str, str]:
    """Rename KB item display name (without changing underlying file/collection id)."""
    normalized_display_name = str(display_name or "").strip()
    if not normalized_display_name:
        raise ValueError("Display name cannot be empty")

    if category in ["collection", "index"]:
        _register_collection_display_name(filename, normalized_display_name)
        return {
            "status": "renamed",
            "category": category,
            "collection": filename,
    ...
    if target_path.is_dir():
        meta = _read_folder_meta(target_path) or {}
        ...
        with open(target_path / "_meta.json", "w", encoding="utf-8") as f:
            json.dump(meta_data, f, ensure_ascii=False, indent=2)
    else:
        _register_display_name(base_dir, target_path.name, normalized_display_name)
...
def _delete_milvus_collection(name: str) -> Dict[str, str]:
    ...
    try:
        ...
        _remove_collection_display_name(name)
        client.close()
